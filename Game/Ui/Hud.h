...
#include "Game/Weapons/WeaponManager.h"
    ...
    float reloadTimer = 0.f, float reloadDuration = 2.5f)
{
    ...
    // ════════════════════════════════════════════════════════
    {
        ...
        dl->AddRectFilled({ hx - 1, hy + fs * 0.72f + 4 },
            { hx + bW + 1, hy + fs * 0.72f + 4 + bH + 2 }, IM_COL32(0, 0, 0, 165), 3.f);
        dl->AddRectFilled({ hx, hy + fs * 0.72f + 5 },
            { hx + bW, hy + fs * 0.72f + 5 + bH }, IM_COL32(22, 22, 20, 240), 2.f);
        if (hpFrac > 0.f)
            dl->AddRectFilled({ hx, hy + fs * 0.72f + 5 },
                { hx + bW * hpFrac, hy + fs * 0.72f + 5 + bH }, hpCol32, 2.f);
        dl->AddRectFilled({ hx, hy + fs * 0.72f + 5 },
            { hx + bW * hpFrac, hy + fs * 0.72f + 5 + bH * 0.4f }, IM_COL32(255, 255, 255, 18));

        std::string hpStr = std::to_string((int)playerHP)
            + " / " + std::to_string((int)playerMaxHP);
        dl->AddText(font, fs, { hx, hy + fs * 0.72f + 13 }, hpCol32, hpStr.c_str());
    }
    ...
    // ════════════════════════════════════════════════════════
    {
        ...
        dl->AddRectFilled({ wx - 10, wy - 6 }, { wx + pW + 4, wy + pH + 6 }, IM_COL32(0, 0, 0, 75), 5.f);
        dl->AddLine({ wx - 10, wy - 6 }, { wx + pW + 4, wy - 6 }, IM_COL32(168, 158, 128, 110), 1.5f);
        ...
        dl->AddText(font, fs * 0.85f, { wx, wy + 4 }, IM_COL32(162, 155, 138, 195), wname.c_str());
        ...
        dl->AddText(font, fs * 0.85f, { wx + amW + 2, wy + 27 }, IM_COL32(100, 95, 88, 195), amRes.c_str());

        float amFrac = def.maxAmmo > 0
            ? std::max(0.f, std::min(1.f, (float)gun.ammo / def.maxAmmo)) : 0.f;
        dl->AddRectFilled({ wx, wy + 48 }, { wx + pW, wy + 52 }, IM_COL32(22, 22, 20, 220), 2.f);
        if (amFrac > 0.f)
    ...
    if (gun.reloading) {
        ...
        dl->AddRectFilled({ rX - 1, rY - 1 }, { rX + rW + 1, rY + rH + 1 }, IM_COL32(0, 0, 0, 130), 3.f);
        dl->AddRectFilled({ rX,   rY }, { rX + rW,   rY + rH }, IM_COL32(22, 22, 20, 220), 2.f);
        dl->AddRectFilled({ rX,   rY }, { rX + rW * rprog, rY + rH }, IM_COL32(210, 178, 70, 255), 2.f);
    ...
    // ════════════════════════════════════════════════════════
    // FPS / позиция / режимы — верхние углы
    // ════════════════════════════════════════════════════════
    float sy = 10.f;
    if (showFPS) {
        ImU32 fc;
