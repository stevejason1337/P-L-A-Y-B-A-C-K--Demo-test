...
struct Console
{
    ...
    void execute(const std::string& cmd)
    {
        ...
        if (name == "help") {
            ...
            if (n == 1) enemyManager.spawnPigDemon(spawnPos);
            else        enemyManager.spawnPigDemonGroup(spawnPos, n, n > 3 ? 5.f : 3.f);
            ...
            print("Goodbye.", CON_YELLOW); gShouldQuit = true;
        }
    ...
    void keyInput(int key, int action)
    {
        ...
        if (action == GLFW_PRESS || action == GLFW_REPEAT) {
            if (key == GLFW_KEY_ENTER) {
                // ImGui InputText с EnterReturnsTrue уже обрабатывает
            }
                ...
                if (histIdx > 0) { histIdx--; input = history[(int)history.size() - 1 - histIdx]; }
            ...
            auto inputCB = [](ImGuiInputTextCallbackData* d) -> int {
                ...
                if (d->EventFlag == ImGuiInputTextFlags_CallbackHistory) {
                    if (d->EventKey == ImGuiKey_UpArrow && !con->history.empty()) {
                        ...
                        d->DeleteChars(0, d->BufTextLen); d->InsertChars(0, h.c_str());
                    }
                ...
                if (d->EventFlag == ImGuiInputTextFlags_CallbackCompletion) {
                    ...
                    con->autoComplete();
                    d->DeleteChars(0, d->BufTextLen); d->InsertChars(0, con->input.c_str());
                }
                return 0;
                };
