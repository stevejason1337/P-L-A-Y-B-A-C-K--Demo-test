...
int main()
{
    ...
    if (cfgExists) loadEngineConfig();
    else           showAPISelectMenu();
    ...
    // Спавн из карты
    {
        ...
        if (mf) {
            ...
            while (std::getline(mf, line)) {
                std::istringstream ss(line); ss >> cmd;
    ...
    for (int i = 0; i < (int)weaponManager.models.size(); i++) {
        ...
        if (mc == 0)
            printf("[WEAPON] ERROR #%d '%s' = 0 meshes!\n", i, weaponDefs[i].file.c_str());
        else
    ...
    while (!glfwWindowShouldClose(window) && !gShouldQuit)
    {
        ...
        if (playerHP <= 0.f && !godMode) {
            ...
            ImGui::SetNextWindowPos({ dsz.x * 0.5f, dsz.y * 0.5f }, ImGuiCond_Always, { 0.5f,0.5f });
