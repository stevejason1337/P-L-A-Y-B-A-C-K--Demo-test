...
struct EnemyManager
{
        ...
        if (!pigDemonPath.empty()) {
            if (!gModelPigDemon.load(pigDemonPath, pigDemonTexDir)) {
                ...
                if (!pigDemonAnimFbx.empty()) {
                    ...
                    if (s.WALK.empty())
                        s.WALK = _findAnim({ "Walk_Fwd","MF_Walk_Fwd","ThirdPersonWalk","walking","Walk","Run","run","ThirdPersonRun" });

                    std::string runAnim = _findAnim({ "Run_Fwd","MF_Run_Fwd","ThirdPersonRun","Sprint","sprint","Run","run" });
                    ...
                    if (s.RELOAD.empty())
                        s.RELOAD = _findAnim({ "High_Kick","High Kick","Fist_Fight","Fist Fight","Attack_02","Sprint","sprint" });
                    if (s.RELOAD.empty()) s.RELOAD = s.SHOOT;

                    if (s.IDLE.empty())
                        s.IDLE = _findAnim({ "Combat_Idle","Combat Idle","Idle","idle","ThirdPersonIdle","MF_Idle","Stand" });
                    if (s.DEATH.empty())
                        s.DEATH = _findAnim({ "Knock_Out","Knock Out","KnockOut","Death","death","Dying","dying","Dead","ThirdPersonDeath","MF_Death" });
                    if (s.HIT.empty())
                        s.HIT = _findAnim({ "Damaged","damaged","Hit_React","HitReact","hit reaction","Hit","hit","Getting Hit","ThirdPersonHit","MF_Hit" });
    ...
    void _spawnEnemy(glm::vec3 p, EnemyType t)
    {
        ...
        if (t == EnemyType::ZOMBIE) { e.hp = 180.f; e.meleeRange = 2.0f; e.scale = 0.01f; }
        else if (t == EnemyType::ZOMBIE2) { e.hp = 150.f; e.meleeRange = 2.0f; e.scale = 0.01f; }
        else if (t == EnemyType::PIG_DEMON) {
            e.hp = 350.f; e.meleeRange = 2.5f; e.scale = 0.01f; e.animSpeed = 1.0f;
        }
        ...
        const glm::vec3& shootDir = glm::vec3(0, 0, 1))
    {
        ...
        if (idx >= 0) {
            glm::vec3 hitP = orig + dir * best;
            enemies[idx].takeDamage((float)bestDmg, shootDir);
            if (dmgOut) *dmgOut = best;
        }
        return idx;
    }
};
